...
class LeRobotCobotDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        data_transforms = _transforms.Group(
            inputs=[cobot_policy.CobotInputs(adapt_to_pi=self.adapt_to_pi)],
            outputs=[cobot_policy.CobotOutputs(adapt_to_pi=self.adapt_to_pi)],
        )
        if self.use_delta_joint_actions:
            delta_action_mask = _transforms.make_bool_mask(6, -1, 6, -1)
            data_transforms = data_transforms.push(
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_lerobot_combined_data",
        model=pi0_config.Pi0Config(pi05=True),
        data=LeRobotCobotDataConfig(
            repo_id="pi05_lerobot_combined_data",
            assets=AssetsConfig(
                assets_dir="/workspace/openpi/assets/pi05_lerobot_combined_data",
                asset_id="pi05_lerobot_combined_data",
            ),
            repack_transforms=_transforms.Group(
                inputs=[
                    _transforms.RepackTransform(
