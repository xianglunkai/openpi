...
def convert_weights(weights, dump_weights):
    ...
    jax_array_w = dump_weights['PaliGemma']['img']['embedding']['kernel']['value']
    jax_array_b = dump_weights['PaliGemma']['img']['embedding']['bias']['value']
    jax_array_e = dump_weights['PaliGemma']['img']['pos_embedding']['value']
    numpy_array_w = np.array(jax_array_w)
    numpy_array_b = np.array(jax_array_b)
    numpy_array_e = np.array(jax_array_e)
    numpy_array_w.flags.writeable = True  # 确保数组可写
    numpy_array_b.flags.writeable = True
    numpy_array_e.flags.writeable = True
    ...
    weights['vision_patch_embedding_w'].copy_(torch.tensor(numpy_array_w, dtype=torch.bfloat16, device="cpu"))
    weights['vision_patch_embedding_b'].copy_(torch.tensor(numpy_array_b, dtype=torch.bfloat16, device="cpu"))
    weights['vision_position_embedding'].copy_(torch.tensor(numpy_array_e, dtype=torch.bfloat16, device="cpu").squeeze())

    vision_attn_qkv_w = torch.cat([
        torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['query']['kernel']['value']), dtype=torch.bfloat16, device="cpu").flatten(2),
        torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['key']['kernel']['value']), dtype=torch.bfloat16, device="cpu").flatten(2),
        torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['value']['kernel']['value']), dtype=torch.bfloat16, device="cpu").flatten(2),
    ], dim=2)
    weights['vision_attn_qkv_w'].copy_(vision_attn_qkv_w)
    vision_attn_qkv_b = torch.cat([
        torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['query']['bias']['value']), dtype=torch.bfloat16, device="cpu").flatten(1),
        torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['key']['bias']['value']), dtype=torch.bfloat16, device="cpu").flatten(1),
        torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['value']['bias']['value']), dtype=torch.bfloat16, device="cpu").flatten(1),
    ], dim=1)
    weights['vision_attn_qkv_b'].copy_(vision_attn_qkv_b)
    weights['vision_attn_o_w'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['out']['kernel']['value']), dtype=torch.bfloat16, device="cpu").flatten(1, -2))
    weights['vision_attn_o_b'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MultiHeadDotProductAttention_0']['out']['bias']['value']), dtype=torch.bfloat16, device="cpu").flatten(1))

    weights['vision_ffn_up_w'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MlpBlock_0']['Dense_0']['kernel']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_ffn_up_b'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MlpBlock_0']['Dense_0']['bias']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_ffn_down_w'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MlpBlock_0']['Dense_1']['kernel']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_ffn_down_b'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['MlpBlock_0']['Dense_1']['bias']['value']), dtype=torch.bfloat16, device="cpu"))

    weights['vision_pre_attn_norm_w'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['LayerNorm_0']['scale']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_pre_attn_norm_b'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['LayerNorm_0']['bias']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_pre_ffn_norm_w'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['LayerNorm_1']['scale']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_pre_ffn_norm_b'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoderblock']['LayerNorm_1']['bias']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_final_norm_w'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoder_norm']['scale']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['vision_final_norm_b'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['Transformer']['encoder_norm']['bias']['value']), dtype=torch.bfloat16, device="cpu"))

    # encoder weights
    weights['encoder_multi_modal_projector_w'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['head']['kernel']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['encoder_multi_modal_projector_b'].copy_(torch.tensor(np.array(dump_weights['PaliGemma']['img']['head']['bias']['value']), dtype=torch.bfloat16, device="cpu"))
    ...
    # decoder weights
    # print("==================================================================")
    # print(dump_weights['PaliGemma']['llm']['layers']['pre_attention_norm_1']['Dense_0']["bias"].keys())
    # print(dump_weights['PaliGemma']['llm']['layers']['pre_attention_norm_1']['Dense_0']['kernel'].keys())
    try:
        w_scale = dump_weights['PaliGemma']['llm']['layers']['pre_attention_norm_1']['scale']['value'].astype('float32')
    except KeyError as e:
        w_scale = np.zeros((18, 1024), dtype='float32')  # 如果没有 scale，使用零填充
    ...
    try:
        ...
        rms_norm = np.zeros((18, 1024), dtype='float32')  # 如果没有 scale，使用零填充
    ...
    # print("===========================dump_weights=======================================")
    # print(dump_weights.keys())
    weights['decoder_state_in_proj_w'].copy_(torch.tensor(np.array(dump_weights['action_in_proj']['kernel']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['decoder_state_in_proj_b'].copy_(torch.tensor(np.array(dump_weights['action_in_proj']['bias']['value']), dtype=torch.bfloat16, device="cpu"))
    ...
    mlp_in_weight_action = torch.tensor(
        np.array(dump_weights['time_mlp_in']['kernel']['value'][:1024, :]),
        dtype=torch.bfloat16, device="cpu"
    )
    ...
    action_time_mlp_in_b = torch.tensor(
        np.array(dump_weights['time_mlp_in']['bias']['value']),
        dtype=torch.bfloat16, device="cpu"
    )
    action_in_proj_w = torch.tensor(
        np.array(dump_weights['action_in_proj']['kernel']['value']),
        dtype=torch.bfloat16, device="cpu"
    )
    action_in_proj_b = torch.tensor(
        np.array(dump_weights['action_in_proj']['bias']['value']),
        dtype=torch.bfloat16, device="cpu"
    )
    decoder_action_fused_out_proj_w = torch.tensor(
        np.array(dump_weights['action_out_proj']['kernel']['value']),
        dtype=torch.bfloat16, device="cpu"
    )
    decoder_action_fused_out_proj_b = torch.tensor(
        np.array(dump_weights['action_out_proj']['bias']['value']),
        dtype=torch.bfloat16, device="cpu"
    )
    try:
        final_norm_scale = torch.tensor(
            np.array(dump_weights['PaliGemma']['llm']['layers']['final_norm']['scale']['value']),
            dtype=torch.bfloat16, device="cpu"
        )
    except KeyError as e:
        final_norm_scale = torch.zeros((1024), dtype=torch.bfloat16, device="cpu")  # 如果没有 scale，使用全1填充
    ...
    for t in range(n_decode_steps):
        ...
        time_tensor = torch.tensor(np.array([time_val]), device="cpu")
        time_emb = _create_sinusoidal_pos_embedding(time_tensor, 1024, 4e-3, 4.0, "cpu").squeeze(0)
        time_emb = time_emb.to(torch.bfloat16)
        if mlp_in_weight_time.shape[0] == 0:
            mlp_in_weight_time = torch.zeros((1024, 1), dtype=torch.bfloat16, device="cpu")
    ...
    weights['decoder_action_mlp_w'].copy_(torch.tensor(np.array(dump_weights['time_mlp_out']['kernel']['value']), dtype=torch.bfloat16, device="cpu"))
    weights['decoder_action_mlp_b'].copy_(torch.tensor(np.array(dump_weights['time_mlp_out']['bias']['value']), dtype=torch.bfloat16, device="cpu"))
...
def load_jax_weights(jax_path: str):
    ...
    print(f"Loading JAX weights from: {jax_path}")
    ...
    #     params = ckptr.restore(
    #         params_path,
    #         item=jax.tree.map(lambda x: jax.ShapeDtypeStruct(x.shape, x.dtype, sharding=single_device_sharding))
    #     )
...
def prepare_prompt(prompt: str, embedding_weight, tokenizer_path):
    ...
    embedding_weight_np = np.array(embedding_weight)
    embedding_weight_np.flags.writeable = True  # 确保数组可写
