...
def main() -> None:
    ...
    print(f"[launch_ros1_agilex_rollout] waiting for actor_service at {actor_service_url}", flush=True)
    _wait_for_http(f"{actor_service_url.rstrip('/')}/version")
    print(f"[launch_ros1_agilex_rollout] waiting for replay_manager at {replay_service_url}", flush=True)
    _wait_for_http(f"{replay_service_url.rstrip('/')}/stats")
    print("[launch_ros1_agilex_rollout] services ready, starting ROS1 AgileX rollout.", flush=True)

    os.chdir(REPO_ROOT)
    os.execv(sys.executable, [sys.executable, str(PIKA_SYNC_ROS1_AGILEX), *argv])
