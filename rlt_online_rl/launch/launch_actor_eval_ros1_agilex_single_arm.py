...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch ROS1 AgileX eval rollout with actor_service only.")
...
def main() -> None:
    ...
    print(f"[launch_actor_eval_ros1] using config {config_path}", flush=True)
    print(f"[launch_actor_eval_ros1] waiting for actor_service at {actor_service_url}", flush=True)
    _wait_for_http(f"{actor_service_url.rstrip('/')}/version")
    print("[launch_actor_eval_ros1] actor_service ready, starting ROS1 eval rollout.", flush=True)

    argv = [
        sys.executable,
        str(PIKA_SYNC_ROS1_AGILEX),
        "--config",
        config_path,
        "--eval_actor_only",
        *args.remaining,
